...
```
stdin JSON → json.Decode(StdinData) → ComputeMetrics → GetGitInfo → GetUsage → ParseTranscript → GetAccountInfo → Render → stdout (ANSI lines, spaces → NBSP)
```
...
95.6% coverage. Every source file has a `_test.go` pair plus `integration_test.go` for full pipeline tests.
...
- **Config merging**: `mergeFeatures(base, override)` is additive-only. Override `true` enables; `false` preserves base value. No reflection — explicit per-field.
